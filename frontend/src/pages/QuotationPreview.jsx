...
export default function QuotationPreview() {
 ...
 // แทนที่ handleDownloadExcel ทั้งหมดใน QuotationPreview.jsx
...
const handleDownloadExcel = async () => {
  ...
  // ── Logo row ──
  boxStart = r; // outer box starts at logo
  try {
    const logoRes = await fetch('/NT logo.png');
    const logoBuffer = await logoRes.arrayBuffer();

    // เช็ค aspect ratio จริงก่อน embed
  ...
  // ── Info section: left (A:C) | right (D:F) ──
  const leftInfo = [
  ...
  // ── Signature space (4 rows) — outer border only ──
  for (let s = 0; s < 4; s++) {
    sheet.mergeCells(`A${r}:B${r}`);
    sheet.mergeCells(`C${r}:D${r}`);
    sheet.mergeCells(`E${r}:F${r}`);
  ...
  sheet.getRow(r).height = 16;
  r++;
  const boxEnd = r - 1; // last content row before spacer

  // ── Outer border around entire quotation box ──
  const COLS = 6;
  ...
  // ── Footer: yellow background (left | right) ──
  ...
  const FOOTER_CONTACT       = 'Contact Center 1888';
  const FOOTER_TAX           = 'เลขประจำตัวผู้เสียภาษีอากร / Tax ID : 0107564000014';
  const FOOTER_NT_REF        = 'NT001 รหัสพัสดุ 10065112 กระดาษจดหมาย (ใช้ภายนอก) หน่วยนับ BK.';
  const FOOTER_PRINT         = 'พิมพ์ที่ : ศูนย์บริการสั่งพิมพ์ บริษัท โทรคมนาคมแห่งชาติ จำกัด (มหาชน) โทร. 0 2591 8042';

  const yellowFill = { type: 'pattern', pattern: 'solid', fgColor: { argb: YELLOW } };

  // yellow row — left A:C, right D:F
  sheet.mergeCells(`A${r}:C${r}`);
  sheet.mergeCells(`D${r}:F${r}`);
  ...
  fyLeft.value = `${FOOTER_ADDRESS_LINE1}\n${FOOTER_ADDRESS_LINE2}`;
  ...
  fyRight.value = `${FOOTER_WEBSITE}  |  ${FOOTER_CONTACT}\n${FOOTER_TAX}`;
  fyRight.font = { size: 9, name: 'Arial' };
  fyRight.alignment = { horizontal: 'right', vertical: 'middle', wrapText: true };
  fyRight.fill = yellowFill;

  // fill middle cells yellow too
  [2, 3, 5, 6].forEach(col => {
    const c = fyRow.getCell(col);
    c.fill = yellowFill;
  });
  r++;

  // ── Footer note: left A:C, right D:F ──
  sheet.mergeCells(`A${r}:C${r}`);
  sheet.mergeCells(`D${r}:F${r}`);
